...
import IVibconnectQuery from "../types/queryFromVibconnect";
import ConferenceCallBacksModel from "../models/ConferenceCallBacksModel";
import { deleteFile } from "../middleware/fileUploadToS3";
...
import {
  generateXmlForPlayNode,
  generateXmlForIvrNode,
  generateXmlForMessage,
  generateXmlForMultiPartyCallNode,
  getOrCreateContact,
  generateXMLForStickyAgentNode,
} from "../helper/callXmlGenerator";
...
export default class IvrStudiosController extends Controller {
  ...
  public async xmlGenerator (req :IRequest , res : IResponse ) : Promise<any>{
  ...
  public async downloadRecording(req: IRequest, res: IResponse): Promise<any> {
    ...
    const fileData: any = await this.downloadFile(
      config.AWS_BUCKET_NAME_RECORDINGS!,
      fileLocation
    );
  ...
  public async deleteAudioFromS3(req: IRequest, res: IResponse): Promise<any> {
    ...
    await deleteFile(bucketName, `public/${fileName}`);
  ...
  public async getVibconnectDataAndModefiy(
    req: IRequest,
    res: IResponse
  ): Promise<any> {
    const actionPath = req.params.action;
    const queryFromVibconnect = req.query as Pick<
      IVibconnectQuery,
      | "AccountSid"
      | "ApiVersion"
      | "CallSid"
    ...
    const callStatusFromApiCall = await ConferenceModel.findOne(myQuery);
    ...
    if (updatedQueryObjectForVibtree) {
      const updatedQueryForVibtree = this.convertObjectToQueryString(
        updatedQueryObjectForVibtree
      );
      console.log(
        "address : ",
        `${conf.BaseUrl}/api/action/${actionPath}/?${updatedQueryForVibtree}`
      );
      return res.redirect(
        `${conf.BaseUrl}/api/action/${actionPath}/?${updatedQueryForVibtree}`
      );
  ...
  public async checkIfCustomerInLine(
    req: IRequest,
    res: IResponse
  ): Promise<any> {
    ...
    try {
      const callDetailsOfConferenceSid = await ConferenceCallBacksModel.find({
      ...
      for (let i = 0; i < callDetailsOfConferenceSid.length; i++) {
        if (
          callDetailsOfConferenceSid[i].CallSid === parentCallSid &&
      ...
      console.log(
        "correct conference id in which customer is in line : ",
        correctConferenceIdInWhichCustomerIsInLine
      );
      console.log(
        "count of member in conference : ",
        countOfMemberInConference
      );
      console.log(
        "count of member leave conference : ",
        countOfMemberLeaveConference
      );
      if (
        countOfMemberInConference >= 1 &&
        ...
        if (req.params.url) {
          if (req.params.url !== "empty") {
            ...
            if(req.params.url.includes('SendDigits')){
              ...
              logger.info('send digits conference url : ' + voice.toString())
              return res.send(voice.toString());
            }
  ...
  public async getConferenceRoom(req: IRequest, res: IResponse): Promise<any> {
  ...
  public async sendCompletedMessageThree(
    auth_id: any,
    auth_secret: any,
    from: any,
    pe_id: any,
    ...
    const link: string = `https://api.vibconnect.io/v1/Accounts/${auth_id}/Messages`;
    ...
    const correctFormatOfCustomerNumber =
      this.checkTheNumberContainsSymbolOrNOT(to);
  ...
  public async sendMessageToDomestic(
    auth_id: any,
    auth_secret: any,
    from: any,
    pe_id: any,
  ...
  updateRealTimeDataOfIvrStudiousForApiCall = async (
    query: any,
    updates: any
  ) => {
  ...
  loopOverNumberToGetAvailableNumberOfAgent = async (
    arrayOfAgentNumbers: any[]
  ) => {
    ...
    const myQuery = onlyNumbersArray
      ...
      : {};
    const dataInRealTime: any = await IvrStudiosRealTime.find(myQuery);
    console.log("data in real time : ", dataInRealTime);

    const numbersArrayInRealTime = dataInRealTime.map((val: any) => {
      return val.Receiver;
    });
    ...
    const x = numberArrayInDataBase.filter(
      (val) => !numbersArrayInRealTime.includes(val)
    );
    console.log("x : ", x);
    if (x.length > 0) {
      return x[0];
  ...
  sleep = async (fn: any, ...args: any) => {
    await this.timeout(3000);
    return fn(...args);
  };

  getConferenceSidAndEndConference = async (
    ParentCallSid: string,
    authId: string,
    authSecretid: string
  ) => {
    const conferenceDetailsOfParticularCall: any =
      await this.findConferenceIdUsingParentCallId(
        ParentCallSid,
        authId,
        authSecretid
      );
    console.log(
      "conferenceDetailsOfParticularCall : ",
      typeof conferenceDetailsOfParticularCall,
      conferenceDetailsOfParticularCall
    );
  ...
  findConferenceIdUsingParentCallId = async (
    parentCallId: string,
    authId: string,
    authSecretId: string
  ) => {
    ...
    const result: any = await this.getListOfConference(authId, authSecretId);
    const jsonResult = JSON.parse(result);
    // console.log("result : ", typeof jsonResult, jsonResult)
    const conferenceDetailsOfParticularCall = jsonResult?.conferences.filter(
      (val: any) => val.friendly_name === `Room_${parentCallId}`
    );
  ...
  handleGenerateRecordXml = async (
    targetNode: any,
    detailsFromVibconnect: any,
    id: any,
    nextNode: any
  ) => {
    const voice: VoiceResponse | any = new VoiceResponse();
    ...
    if (callSid !== null && callSid !== undefined) {
      const record = new VoiceMailRecordModel(data);
      await record.save();
    }
    try {
      voice.record({
  ...
  checkChildCallStatusOfPreviousCall = async (parentCallSid: any) => {
    let result = { status: false, data: {} };
    const myQuery = { ParentCallSid: parentCallSid };
    const findDetails = await ConferenceModel.find(myQuery);
  ...
  handleMultiPartyCallDistributionOfTypeRoundRobin = async (
    targetNode: any,
    dataFromVibconnect: any,
    id?: any
  ) => {
    const voice: VoiceResponse | any = new VoiceResponse();
    ...
    try {
      ...
      if (targetNode.data.mpcCallUsing === "User") {
        ...
        const numbersFromUerID = await this.extractAllNumberOfCorrespondingUser(
          detailsOfNumbersFromUI
        );
      ...
      if (nextNumberIfbusy === "no-number") {
        ...
        console.log(
          "data_required_to_filter_conference_details 3901 : ",
        ...
        const responseOfConferenceEnd = this.sleep(
          this.getConferenceSidAndEndConference,
          dataFromVibconnect.ParentCallSid,
          authId,
          authSecretid
        );
        ...
        console.log(
          "node credentials : ",
          targetNode.data.authId,
          " : ",
          targetNode.data.authSecret
        );

        const call_details = await this.MakeConferenceCall(
          targetNode.data.authId,
          targetNode.data.authSecret,
          body
        );
        ...
        this.updateRealTimeDataOfIvrStudiousForApiCall(
          queryToSend,
          updateToSend
        );
      }
  ...
  handleMultiPartyCallDistributionProcess = async (
    targetNode: any,
    dataFromVibconnect: any,
    id?: any
  ) => {
    ...
    try {
      ...
      purchasedNumber = this.checkTheNumberContainsSymbolOrNOT(
        dataFromVibconnect.To
      );
      ...
      if (targetNode.data.mpcCallUsing === "Number") {
        await Promise.all(
          targetNode.data.mpcCallUsingNumbers.map(async (number: any) => {
            const body: any = {
              statusCallback: `${conf.BaseUrl}/api/webhook/vibconnect/ivr_studios/api_call/parallel`, // it also save data to ivrStudios dataBase but only for API-Call
              statusCallbackEvent: "initiated, ringing, answered, completed",
              Record: "true",
  ...
  runApiRequestInBackground = async (
    detailsOfTargetNode: any,
    id: any,
    customer?: any
  ) => {
    const request: any = await this.collectDetailsAndMakeRequest(
      detailsOfTargetNode,
      customer
    );
    ...
    const exactValueAfterFilter = this.filterVariablesFromResponse(
      variableArray,
      jsonResponse
    );
    ...
    await IvrStudiosRealTime.findOneAndUpdate(
      { ParentCallSid: id },
      { $push: { variables: exactValueAfterFilter } }
    );
  };

  findTargetsFromSource(allNodes: any, source: string): any {
    let sourceNodes: any = allNodes.filter((node: any) => {
  ...
  convertArrayToJson(arr: any, customer?: any) {
    // console.log("arr : ",arr)
    ...
    //@ts-ignore
    var replace_with: any = removed_sip; // here we are replacing it
    if (arr.includes(val_to_replace)) {
      const x = arr.map((item: any) => {
  ...
  async collectDetailsAndMakeRequest(details: any, customer?: any) {
    const { functionBody, requestUrl, requestMethod, headerArrays } =
      details.data;
  ...
  destructuringNestedArrayAndGiveValue(
    array: any,
    responseObject: any,
    key: any
  ) {
  ...
  filterVariablesFromResponse(variableArray: any, responseObject: any) {
    ...
    for (const i in variableArray) {
      const x = this.destructuringNestedArrayAndGiveValue(
        variableArray[i].variable_value.split("."),
        responseObject,
  ...
  removeFunctionNodeIdFromNextNode(allNodes: any, filteredNode: any) {
    ...
    for (const i in allNodes) {
      for (const j in filteredNode) {
        if (
          filteredNode[j].target === allNodes[i].id &&
    ...
    if (data.timeArray !== undefined) {
      if (data.timeArray.length > 0) {
        for (let i = 0; i < data.timeArray.length; i++) {
          let start = moment(data.timeArray[i].startTime, "HH:mm:ss").format();
          let end = moment(data.timeArray[i].endTime, "HH:mm:ss").format();
          ...
          if (moment(currentTime).isBetween(start, end)) {
            ...
            if (data.timeArray[i].startTime !== undefined) {
              startDate = moment(data.timeArray[i].startTime , "HH:mm:ss" ).format();
            }
            if (data.timeArray[i].endTime !== undefined) {
              endDate = moment(data.timeArray[i].endTime, "HH:mm:ss").format();
            }
          }
        }
      }
    } else {
      startDate = moment(data.startTime, "HH:mm:ss").format();
      endDate = moment(data.endTime, "HH:mm:ss").format();
    }
  ...
  public async createIvrFlowAccordingToUIFlowVersionTwo(
    req: IRequest,
    res: IResponse
  ): Promise<any> {
    res.set("Content-Type", "text/xml");
    const defaultVri: VoiceResponse | any = new VoiceResponse();
    const authID: any = req.query.AccountSid;
    let purchasedNumber: any = req.query.To;
    ...
    timeAfterThreeHour = moment().add(3, "hours").toDate();
    ...
    try {
      const voice: VoiceResponse | any = new VoiceResponse();
      const data: any = await ivrFlowUIModel.findOne({
        auth_id: authID,
        number: purchasedNumber,
      });
      if(!data?.active){
        logger.warn(`WorkFlow of incoming call is not active in -> ${purchasedNumber}`)
        voice.hangup()
        return res.send(voice.toString())
      }
      if(!data?.haveCredits){
        logger.warn(`No credits in Account ${authID} in number -> ${purchasedNumber}`)
        voice.hangup()
        return res.send(voice.toString())
      }
      //@ts-ignore
      ...
      //@ts-ignore
      const connectors: any = data.input.filter((item: any) => {
        if (item.type == "buttonedge") {
          return item;
        }
      });
      //@ts-ignore
      const firstNode = data.input.find((item: any) => {
      ...
      //@ts-ignore
      let sourceId_to_find_in_next_connector: any = firstNode.id;

      const nextNode = connectors.find((item: any) => {
      ...
      const nextNodeData = data.input?.find((item: any) => {
      ...
      if (nextNodeData.type === "PlayAudioNode") {
        const newRealTime = new IvrStudiosRealTime({
          ...req.query,
          source: "Web",
        ...
        const xmlFromHelper = generateXmlForPlayNode(
          document_id,
          nextNodeId,
      ...
      if (nextNodeData.type === "ivrNode") {
        ...
        await newRealTime.save();
        const xml = generateXmlForIvrNode(
          document_id,
          nextNodeId,
          sourceId_to_find_in_next_connector,
          nextNodeData.data
        );
        return res.send(xml);
      }
            ...
            for (let i = 0; i < nodeDetails.bHourDays.length; i++) {
              if (nodeDetails.bHourDays[i].isWorking === true) {
                ...
                if (
                  ...
                  const tempStartEndObj =
                    await this.getStartAndEndTimeFromMultipleTimeInSingleDay(
                      nodeDetails.bHourDays[i],
                      now
                    );
                  ...
                  if (moment(now).isBetween(start, end)) {
                    ...
                    const nextNodeConnectedToBussinessHourNodeOn =
                      connectors.filter((item: any) => {
                        if (
                          item.source === bussinessHourNodeId &&
                    ...
                    const nextNodeIfSuccess = data.input?.find((item: any) => {
                      if (
                        item.id ===
                    ...
                    if (nextNodeIfSuccess) {
                      ...
                      if (nextNodeIfSuccess.type === "ivrNode") {
                        ...
                        const xml = generateXmlForIvrNode(
                          document_id,
                          nextNodeIfSuccess.id,
                          sourceId_to_find_in_next_connector,
                          nextNodeIfSuccess.data
                        );
                        return res.send(xml);
                      }
                      ...
                      if (nextNodeIfSuccess.type === "PlayAudioNode") {
                        ...
                        await newRealTime.save();
                        const nextNode_for_action = connectors.find(
                          (item: any) => {
                        ...
                        const nextNode_for_action_id_data =
                          nextNode_for_action.target;
                        const xmlFromHelper = generateXmlForPlayNode(
                          document_id,
                          nextNodeIfSuccess.id,
                          nextNode_for_action_id_data,
                      ...
                      if (nextNodeIfSuccess.type === "MultiPartyCallNode") {
                        ...
                        await newRealTime.save();
                        const dataFromVibconnect: any = req.query;
                        const xml = await generateXmlForMultiPartyCallNode(
                          document_id,
                          nextNodeIfSuccess.id,
                          nextNodeIfSuccess.id,
                          nextNodeIfSuccess.data,
                      ...
                      if (nextNodeIfSuccess.type === "MessageNode") {
                        ...
                        if (
                          nextNodeConnectedToBussinessHourNodeOn.length === 1
                        ) {
                          const dataFromVibconnect: any = req.query;
                          const xml = await generateXmlForMessage(
                            nextNodeIfSuccess.data,
                            dataFromVibconnect
                          );
                          return res.send(xml);
                        ...
                        if (nextNodeConnectedToBussinessHourNodeOn.length > 1) {
                          ...
                          for (
                            let i = 0;
                            i < nextNodeConnectedToBussinessHourNodeOn.length;
                            i++
                          ) {
                            const detailsOfTargetNode = data.input?.find(
                              (node: any) =>
                                node.id ===
                      ...
                      if (nextNodeIfSuccess.type === "BusinessHourNode") {
                        ...
                        if (nextNodeIfSuccess) {
                          if (nodeDetails.bHourOption == "anytime") {
                            ...
                            for (
                              let i = 0;
                              i < nextNodeIfSuccess.data.bHourDays.length;
                              i++
                            ) {
                              if (nodeDetails.bHourDays[i].isWorking === true) {
                                const currentTime = new Date();
                                const currentDay = this.getNameOfDay(
                                  currentTime.getDay()
                                );
                                // console.log(
                                //   "currentDay : ",
                                ...
                                if (
                                  ...
                                  if (moment(now).isBetween(start, end)) {
                                    ...
                                    const nextNodeIfSuccess =
                                      data.input?.filter((item: any) => {
                                        if (
                                          item.id ===
                                    ...
                                    if (nextNodeIfSuccess.length > 0) {
                                      ...
                                      // );
                                      if (
                                        nextNodeIfSuccess[0].type === "ivrNode"
                                      ) {
                                        const newRealTime =
                                          new IvrStudiosRealTime({
                                            ...req.query,
                                            source:
                                              "Web",
                                            expireDate: timeAfterThreeHour,
                                      ...
                                      if (
                                        nextNodeIfSuccess[0].type ===
                                        "PlayAudioNode"
                                      ) {
                                        const nextNode_for_action =
                                          connectors.find((item: any) => {
                                            if (
                                              item.source ===
                                              nextNodeIfSuccess[0].id
                                            ) {
                                      ...
                                      if (
                                        ...
                                        if (
                                          nextNodeConnectedToBussinessHourNodeOn.length ===
                                          1
                                        ) {
                                          const dataFromVibconnect: any =
                                            req.query;
                                          const xml =
                                            await generateXmlForMessage(
                                              nextNodeIfSuccess[0].data,
                                              dataFromVibconnect
                                            );
                                          return res.send(xml);
                                      ...
                                      if (
                                        ...
                                        const dataFromVibconnect: any =
                                          req.query;
                                        const xml =
                                          await generateXmlForMultiPartyCallNode(
                                            document_id,
                                            nextNodeIfSuccess[0].id,
                                            nextNodeIfSuccess[0].id,
                                            nextNodeIfSuccess[0].data,
                                    ...
                                    const nextNodeConnectedToBussinessHourNodeOff =
                                      connectors.filter((item: any) => {
                                        if (
                                          item.source === bussinessHourNodeId &&
                                    ...
                                    const nextNodeIfSuccess =
                                      data.input?.filter((item: any) => {
                                        for (
                                          let i = 0;
                                          i <
                                          nextNodeConnectedToBussinessHourNodeOff.length;
                                          i++
                                        ) {
                                          if (
                                            item.id ===
                                            nextNodeConnectedToBussinessHourNodeOff[
                                              i
                                            ].target
                                          ) {
                                            return item;
                    ...
                    const nextNodeIfSuccess = data.input?.filter(
                      (item: any) => {
                        for (
                          let i = 0;
                          i < nextNodeConnectedToBussinessHourNodeOff.length;
                          i++
                        ) {
                          if (
                            item.id ===
                    ...
                    if (nextNodeIfSuccess[0].type === "PlayAudioNode") {
                      ...
                      if (
                        nextNodeConnectedToBussinessHourNodeOff.length === 1
                      ) {
                        const nextNode_for_action = connectors.find(
                          (item: any) => {
                      ...
                      if (nextNodeIfSuccess.length > 0) {
                        if (nodeDetails.bHourOption == "anytime") {
                          ...
                          if (nextNodeIfSuccess.length > 0) {
                            ...
                            if (nextNodeIfSuccess[0].type === "PlayAudioNode") {
                              ...
                              if (
                                ...
                                const nextNode_for_action = connectors.find(
                                  (item: any) => {
                                    if (
                                      item.source === nextNodeIfSuccess[0].id
                                    ) {
                                      return item;
                                    }
                                  }
                              ...
                              if (
                                ...
                                for (
                                  ...
                                  if (
                                    detailsOfTargetNode.type == "MessageNode"
                                  ) {
                                    const dataFromVibconnect: any = req.query;
                                    //not making it async await because it is making API response slow and we dont need to send message first then play
                                    generateXmlForMessage(
                                  ...
                                  if (
                                    detailsOfTargetNode.type == "PlayAudioNode"
                                  ) {
                                    makePlayXml = true;
                                    playNodeDetails = detailsOfTargetNode.data;
                                    playNodeId = detailsOfTargetNode.id;
                            ...
                            if (
                              nextNodeIfSuccess[0].type === "MultiPartyCallNode"
                            ) {
                              const newRealTime = new IvrStudiosRealTime({
                                ...req.query,
                                source: "Web",
                          ...
                          for (
                            let i = 0;
                            i < nextNodeIfSuccess[0].data.bHourDays.length;
                            i++
                          ) {
                            if (nodeDetails.bHourDays[i].isWorking === true) {
                              const currentTime = new Date();
                              ...
                              if (
                                ...
                                if (moment(now).isBetween(start, end)) {
                                  ...
                                  if (
                                    nextNodeIfSuccess[0].type === "MessageNode"
                                  ) {
                                    let makePlayXml = false;
                                    let playNodeDetails;
                                    let playNodeId: any;
      ...
      if (nextNodeData.type === "StickyAgentNode") {
        ...
        const nextNode_for_action_id_data = nextNode_for_action.target;
        const dataFromVibconnect: any = req.query;
        const xml = await generateXMLForStickyAgentNode(
          document_id,
          nextNodeData.id,
          nextNode_for_action_id_data,
          nextNodeData.data,
  ...
  getOrCreateContact = async (data: dataFromVibconnectForIncoming) => {
  ...
  public async getRealTimeData(req: IRequest, res: IResponse): Promise<any> {
    ...
    const authId: any = req.JWTUser?.authId;
    let contact;
    ...
    if (contact) {
      ...
      //@ts-ignore
      realtimeData = await IvrStudiosRealTime.find({...finalFilter,$or: contactFilter,}).limit(Number(req.query.limit)).skip(Number(req.query.offset)).sort(req.query.sort).select(req.query.fields).populate(req.query.populate);
    } else {
      console.log("Contact not provided");
      //@ts-ignore
      realtimeData = await IvrStudiosRealTime.find(finalFilter).limit(Number(req.query.limit)).skip(Number(req.query.offset)).sort(req.query.sort).select(req.query.fields).populate(req.query.populate);
    }
    const data = {
      data: realtimeData,
    };
  ...
  public async numberNotInService(req: IRequest, res: IResponse): Promise<any> {
    const voice: VoiceResponse | any = new VoiceResponse();
    voice.play(
      "https://danish-asghar2909.github.io/mp3.xml/CancelNumber.mp3.mp3"
    );
    return res.send(voice.toString());
  }
  ...
  public async getTargetNodeAndExecuteVersionTwo(
    req: IRequest,
    res: IResponse,
    next: any
  ): Promise<any> {
    ...
    let Digits: any = req.body.Digits;
    ...
    try {
      if (req.body.Digits == "#") {
        ...
        const voice: VoiceResponse | any = new VoiceResponse();
        const data: any = await ivrFlowUIModel.findById(id);
        // console.log("data : ",data)
        const document_id = id;
        const connectors = data.input.filter((item: any) => {
          if ((item.type = "buttonedge")) {
            return item;
          }
        });

        const firstNode = connectors.map((item: any) => {
          if (item.data !== undefined) {
            if (item.data.cloudNumber !== undefined) {
              return item;
            }
        ...
        const multipleSource = this.findTargetsFromSource(
          connectors,
          sourceId_to_find_in_next_connector
        );
        // console.log("multipleSource : ",multipleSource)
        if (multipleSource.length > 1) {
          const targetFunctionNodeFromSourceWithMultipleTarget =
            this.findIfTargetIsFunction(data.input, multipleSource);
          const { variableArray } =
            targetFunctionNodeFromSourceWithMultipleTarget.data;
          ...
          const request: any = await this.collectDetailsAndMakeRequest(
            targetFunctionNodeFromSourceWithMultipleTarget,
            customer
          );
          const jsonResponse = JSON.parse(request);
        ...
        const nextNode = connectors.map((item: any) => {
          if (item.source === sourceId_to_find_in_next_connector) {
            return item;
          }
        });

        var correctNextNode = nextNode.filter(function (el: any) {
          return el != null;
        });
        ...
        const nextNodeId = this.removeFunctionNodeIdFromNextNode(
          data.input,
          nextNodeIdTrial
        );
        //    console.log("nextId : ",nextId)
        const nextNodeData = data.input?.map((item: any) => {
          if (item.id === nextNodeId) {
            return item;
          }
        });

        const filteredNextNodeData: any = nextNodeData.filter(function (
          item: any
        ) {
          return item != null;
        });
        ...
        if (filteredNextNodeData[0].data.audioUrl) {
          const nextNode_for_action = connectors.map((item: any) => {
            if (item.source === nextNodeId) {
              return item;
            }
          });
          const nextNode_for_action_id = nextNode_for_action.filter(
            (node: any) => {
              return node != null;
            }
          );
          ...
          voice.play(
            { loop: filteredNextNodeData[0].data.loop, digits: "wwww3" },
            filteredNextNodeData[0].data.audioUrl
          );
          voice.pause({ length: filteredNextNodeData[0].data.playAudioPause });
          voice.redirect(
            { method: "GET" },
            `${conf.BaseUrl}/api/ivrstudios/convert/${document_id}/${nextNode_for_action_id_data}/${nextNodeId}`
          );
        ...
        if (filteredNextNodeData[0].data.ivrNumberArray.length > 0) {
          ...
          if (filteredNextNodeData[0].data.ivrAudioUrl) {
            Parent.play(
              { loop: filteredNextNodeData[0].data.loop },
              filteredNextNodeData[0].data.ivrAudioUrl
            );
            voice.pause({
        ...
        const targetNode = allNodes.find((node: any) => node.id === target);
        console.log("targetNode : ", targetNode , target);

        if (target.includes("conference")) {
          const roomId = target.split("|")[1];
          console.log("roomId : ", roomId);
          const conferenceIdFromRoomId: any = await ConferenceModel.findOne({
            FriendlyName: roomId,
          });
          ...
          if (conferenceIdFromRoomId) {
            ...
            const tempData: any = await this.checkChildCallStatusOfPreviousCall(
              conferenceIdFromRoomId?.ParentCallSid
            );
            console.log("Temp Data : ", tempData);
            logger.info("status : " +status + " : main call status " +req.query.DialCallStatus + " "+ req.query.CallStatus+ "Child Call Status : " + ChildCallStatus);
            let statusAccordingToUi: string | any;
            if (tempData.status) {
            ...
            switch (status) {
              case "in-progress":
                if (
                  req.query.DialCallStatus == "answered" ||
            ...
            logger.info("Status According To UI : "+ statusAccordingToUi);
            const connectorWithTargetId: any = allNodes.filter((node: any) => {
              if (
                node.source === source &&
            ...
            const targetNode = allNodes.find(
              (node: any) => node.id === connectorWithTargetId[0].target
            );
            ...
            if (
              ...
              await IvrStudiosRealTime.deleteMany(myQuery);
            }

            console.log("target node : ", targetNode);
            if (targetNode) {
              if (targetNode.type == "PlayAudioNode") {
                const nextNodeToAction = allNodes.filter((node: any) => {
                  if (node.source == targetNode.id) {
                    return node;
                  }
                });
                const detailsOfNextNodeToAction = allNodes.filter(
                  (node: any) => {
                ...
                console.log(
                  "detailsOfNextNodeToAction : ",
                  detailsOfNextNodeToAction
                );
                console.log(
                  "nextNodeToAction multi target: ",
                  nextNodeToAction
                );
                let nextTargetNode: any = nextNodeToAction;
                if (nextTargetNode.length > 0) {
                  if (detailsOfNextNodeToAction.length > 0) {
                    if (detailsOfNextNodeToAction[0].type == "HangUpNode") {
                      ...
                      voice.play(
                        { loop: targetNode.data.loop, digits: "wwww3" },
                        targetNode.data.audioUrl
                      );
                      voice.pause({ length: targetNode.data.playAudioPause });
                      voice.redirect(
                        { method: "GET" },
                        `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${nextNodeToAction[0].target}/${targetNode.id}`
                      );
                      console.log("xml is : ", voice.toString());
                      return res.send(voice.toString());
                    }
              ...
              if (targetNode.type === "MessageNode") {
                console.log(
                  "Do we need to send message : ",
                  conferenceIdFromRoomId.isMessageSent
                );
                if (conferenceIdFromRoomId.isMessageSent !== undefined) {
                  if (conferenceIdFromRoomId.isMessageSent === false) {
                    ...
                    const customerVariablesDetails =
                      await IvrStudiosRealTime.findOne({
                    ...
                    if (customerVariablesDetails) {
                      if (customerVariablesDetails.variables) {
                        console.log(
                          "customerVariablesDetails : ",
                          customerVariablesDetails.variables
                        );
                    ...
                    if (connectorWithTargetId.length > 1) {
                      ...
                      for (let i = 0; i < connectorWithTargetId.length; i++) {
                        ...
                        const targetNode = allNodes.find(
                          (node: any) =>
                            node.id === connectorWithTargetId[i].target
                        );
                        ...
                        if (targetNode.type == "MessageNode") {
                          ...
                          const customer: any =
                            await IvrStudiosStatusModel.find(myQuery);
                          // console.log("customer : ",customer)
                          const time = customer[0].subscribeDate;
                          const call_back_with_digits = customer.filter(
                            (customer: any) => {
                              if (
                                customer.source ===
                          ...
                          const array_of_selected_Digits =
                            call_back_with_digits.map((customer: any) => {
                          ...
                          if (
                            call_back_with_digits.slice(-1)[0] === undefined
                          ) {
                            lastDigits = "";
                          }
                          if (
                          ...
                          if (value.smsTo.toLowerCase() === "specific") {
                            ...
                            if (
                              value.carrierType.toLowerCase() === "domestic"
                            ) {
                              Promise.all(
                                value.toNumbers.map(async (to: any) => {
                                  // console.log("to : ",to.number)
                                  const string_result: any =
                                    await this.sendMessageToDomestic(
                                      value.AuthId,
                                      value.AuthSecret,
                                      value.senderId,
                                      value.peId,
                          ...
                          if (value.smsTo.toLowerCase() === "dynamic") {
                            ...
                            if (
                              ...
                              const string_result: any =
                                await this.sendCompletedMessageThree(
                                  value.AuthId,
                                  value.AuthSecret,
                                  value.senderId,
                                  value.peId,
                    ...
                    if (connectorWithTargetId.length == 1) {
                      ...
                      const myQuery: any = { ParentCallSid: ParentCallSid };
                      const customer: any = await IvrStudiosStatusModel.find(
                        myQuery
                      );
                      // console.log("customer : ",customer)
                      const time = customer[0].subscribeDate;
                      const call_back_with_digits = customer.filter(
                        (customer: any) => {
                          if (
                            customer.source === "gather_response_body_ivr_flow"
                          ) {
                            let Digits = customer.Digits;
                            return Digits;
                          }
                      ...
                      if (value.smsTo.toLowerCase() === "specific") {
                        ...
                        if (
                          value.carrierType.toLowerCase() === "international"
                        ) {
                          Promise.all(
                            value.toNumbers.map(async (to: any) => {
                              // console.log("to : ",to.number)
                  ...
                  if (connectorWithTargetId.length == 1) {
                    ...
                    const array_of_selected_Digits = call_back_with_digits.map(
                      (customer: any) => {
                        return customer.Digits;
                      }
                    );
              ...
              if (targetNode.type === "BusinessHourNode") {
                console.log(
                  "Write Logic for bussiness hour after reading call backs of previous calls "
                );
                ...
                const connectors = allNodes.filter((item: any) => {
                  if (item.type == "buttonedge") {
                    return item;
                  }
                });
                ...
                const filteredNextNodeData = this.findTargetsFromSource(
                  allNodes,
                  targetNode.id
                );
                  ...
                  if (nodeDetails.bHourOption == "anytime") {
                    ...
                    if (nextNodeIfSuccess.length > 0) {
                      ...
                      if (nextNodeIfSuccess[0].type === "ivrNode") {
                        ...
                        if (nextNodeIfSuccess[0].data.ivrAudioUrl) {
                          Parent.play(
                            ...
                            nextNodeIfSuccess[0].data.ivrAudioUrl
                          );
                          voice.pause({
                      ...
                      if (nextNodeIfSuccess[0].type === "PlayAudioNode") {
                        ...
                        //     }
                        // })
                        const nextNode_for_action_id = connectors.filter(
                          (item: any) => {
                            if (item.source === nextNodeIfSuccess[0].id) {
                              return item;
                            }
                        ...
                        voice.redirect(
                          { method: "GET" },
                          `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${nextNode_for_action_id_data}/${source}`
                        );
                        return res.send(voice.toString());
                      }
                    }
                      ...
                      if (nodeDetails.bHourDays[i].isWorking === true) {
                        ...
                        if (
                          ...
                          if (moment(now).isBetween(start, end)) {
                            ...
                            console.log(
                              "nextNodeConnectedToBussinessHourNode 2411: ",
                              nextNodeConnectedToBussinessHourNodeOn
                            );
                            ...
                            if (nextNodeIfSuccess.length > 0) {
                              ...
                              if (
                                nextNodeIfSuccess[0].type === "PlayAudioNode"
                              ) {
                                const newRealTime = new IvrStudiosRealTime({
                                  ...req.query,
                                  source: "Web",
                                ...
                                voice.pause({
                                  length:
                                    nextNodeIfSuccess[0].data.playAudioPause,
                                });
                                voice.redirect(
                                  { method: "GET" },
                                  `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${nextNode_for_action_id_data}/${source}`
                              ...
                              if (nextNodeIfSuccess[0].type === "ivrNode") {
                                const nextNodeToAction = allNodes.filter(
                                  (node: any) => {
                                    if (
                                      node.source == nextNodeIfSuccess[0].id
                                    ) {
                                      return node;
                                    }
                                  }
                                ...
                                voice.pause({
                                  length:
                                    nextNodeIfSuccess[0].data.ivrPlayAudioPause,
                                });
                                console.log("xml is : ", voice.toString());
                                return res.send(voice.toString());
                              }
                              ...
                              if (
                                ...
                                if (
                                  targetNode.data.mpcCallDistribustion ===
                                  "RoundRobin"
                                ) {
                                  console.log(
                                    "write the logic of round robin here "
                                  );
                                  let dataFromVibconnect: any =
                                    JSON.stringify(req.body) !== "{}"
                                      ? req.body
                                      : req.query;
                                  const xml =
                                    await this.handleMultiPartyCallDistributionOfTypeRoundRobin(
                                      targetNode,
                                      dataFromVibconnect,
                                      id
                                    );
                                ...
                                if (
                                  ...
                                  let dataFromVibconnect: any =
                                    JSON.stringify(req.body) !== "{}"
                                      ...
                                      : req.query;
                                  const xml =
                                    await this.handleMultiPartyCallDistributionProcess(
                                      targetNode,
                                      dataFromVibconnect,
                                      id
                                    );
                                  ...
                                  Parent.conference(
                                    {
                                      waitUrl:
                                        nextNodeIfSuccess[0].data.mpcAudio,
                                      statusCallback: `${conf.BaseUrl}/api/webhook/vibconnect/conference`,
                                      statusCallbackEvent:
                                  ...
                                  if (
                                    nextNodeIfSuccess[0].data.mpcCallUsing ===
                                    "User"
                                  ) {
                                    console.log(
                                      "this is not number this is user typ calling system"
                                    );
                                    let detailsOfNumbersFromUI =
                                      nextNodeIfSuccess[0].data
                                        .mpcCallUsingNumbers;
                                    const numbersFromUerID =
                                      await this.extractAllNumberOfCorrespondingUser(
                                        detailsOfNumbersFromUI
                                      );
                                    console.log(
                                  ...
                                  if (
                                    nextNodeIfSuccess[0].data.callType ===
                                    "default"
                                  ) {
                                    From = "+912235315936";
                                  }
                                  ...
                                  console.log(
                                    "node credentials : ",
                                    nextNodeIfSuccess[0].data.authId,
                                    " : ",
                                    nextNodeIfSuccess[0].data.authSecret
                                  );
                                  const call_details =
                                    await this.MakeConferenceCall(
                                      nextNodeIfSuccess[0].data.authId,
                                      nextNodeIfSuccess[0].data.authSecret,
                                      body
                                    );
                                  const call_details_json =
                                    JSON.parse(call_details);
                                  ...
                                  console.log(
                                    "data_required_to_filter_conference_details 2783 : ",
                                    data_required_to_filter_conference_details
                                  );
                                  const conference = new ConferenceModel(
                                    data_required_to_filter_conference_details
        ...
        if (target.includes("checkValue")) {
          ...
          for (let i = 0; i < jsonVariablesFromUrl.length; i++) {
            if (jsonVariablesFromUrl[i].sourceHandle == "functionSuccess") {
              ...
              targetOfNodeIfValueOfVariableExists = allNodes.filter(
                (x: any) => {
              ...
              targetOfNodeIfValueOfVariableDoesNotExist = allNodes.filter(
                (x: any) => {
                  if (x.id == jsonVariablesFromUrl[i].target) {
                    return x;
                  }
          ...
          const valuesAvailableFromRequest: any = await IvrStudiosRealTime.find(
            { ParentCallSid: ParentCallSid }
          );
          let tempVariables = [];
          ...
          if (tempVariables.length > 0) {
            ...
            if (detailsOfTargetNode.type === "MessageNode") {
              const customerVariablesDetails = await IvrStudiosRealTime.findOne(
                { ParentCallSid: ParentCallSid }
              );
              let dataBaseVariables = {};
              if (customerVariablesDetails) {
            ...
            if (detailsOfTargetNode.type === "SpeakNode") {
              ...
              const nextNode = allNodes.find(
                (node: any) => node.source === detailsOfTargetNode.id
              );
              console.log("SpeakNode");
              voice.say(y);
              voice.redirect(
                { method: "GET" },
                `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${nextNode.target}/${detailsOfTargetNode.id}`
              );
              console.log("xml is : ", voice.toString());
              return res.send(voice.toString());
            }
            ...
            if (detailsOfTargetNode.type === "PlayAudioNode") {
              ...
              if (jsonVariablesFromUrl.length > 0) {
                for (let i = 0; i < jsonVariablesFromUrl.length; i++) {
                  if (
                    jsonVariablesFromUrl[i].sourceHandle == "functionSuccess"
                  ) {
              ...
              const stringyfy = encodeURIComponent(
                JSON.stringify(jsonVariablesFromUrl)
              );
              // console.log("stringyfy : ",stringyfy)
              voice.play(
                { loop: detailsOfTargetNode.data.loop, digits: "wwww3" },
                detailsOfTargetNode.data.playAudioUrl
              );
              voice.redirect(
                { method: "GET" },
                `${conf.BaseUrl}/api/ivrstudios/convert/${id}/checkValue|${stringyfy}|${variablesFromUrl[2]}/${detailsOfTargetNode.id}`
              );
              console.log("xml is : ", voice.toString());
              return res.send(voice.toString());
            }
            ...
            if (detailsOfTargetNode.type === "ivrNode") {
              ...
              if (nextNodeToAction.length > 0) {
                ...
                ParentGather.play(
                  { loop: detailsOfTargetNode.data.loop },
                  detailsOfTargetNode.data.ivrAudioUrl
                );
                voice.pause({
          ...
          if (tempVariables.length == 0) {
            let detailsOfTargetNode =
              targetOfNodeIfValueOfVariableDoesNotExist[0];
            ...
            if (detailsOfTargetNode.type === "PlayAudioNode") {
              ...
              if (jsonVariablesFromUrl.length > 0) {
                for (let i = 0; i < jsonVariablesFromUrl.length; i++) {
                  if (
                    jsonVariablesFromUrl[i].sourceHandle == "functionFailed"
                  ) {
                    jsonVariablesFromUrl[i].target = nextNode.target;
                  }
                }
        ...
        if (targetNode.type === "VoiceMailNode") {
          ...
          const xml = await this.handleGenerateRecordXml(
            targetNode,
            detailsFromVibconnect,
            id,
            nextNodeToAction[0].target
          );
          console.log("xml is : ", xml);
          return res.send(xml);
        }
        ...
        if (targetNode.type === "PlayAudioNode") {
          const nextNodeToAction = allNodes.find((node: any) => {
            if (node.source == targetNode.id) {
              return node;
            }
          });
        ...
        if (targetNode.type === "ivrNode") {
          const targetNode: any = allNodes.map((node: any) => {
            if (node.source == target && node.sourceHandle === Digits) {
              return node;
            }
          });
          const filterTargetNode = targetNode.filter((node: any) => {
            return node != null;
          });
          // console.log("targetNode if ivr : ", filterTargetNode)

          //latest changes after function
          let detailsOfTargetNode: any;
          if (filterTargetNode.length > 0) {
            detailsOfTargetNode = allNodes.find(
              (node: any) => node.id === filterTargetNode[0].target
            );
          } else {
          ...
          if (detailsOfTargetNode.type === "BusinessHourNode") {
            ...
            if (filteredNextNodeData.length > 0) {
              if (nodeDetails.bHourOption == "anytime") {
                ...
                for (
                  let i = 0;
                  i < detailsOfTargetNode.data.bHourDays.length;
                  i++
                ) {
                  if (nodeDetails.bHourDays[i].isWorking === true) {
                    const currentTime = new Date();
                    ...
                    if (
                      ...
                      if (moment(now).isBetween(start, end)) {
                        ...
                        if (
                          ...
                          const dataFromVibconnect: any = req.body
                            ? req.body
                            : req.query;
                          const xml = await generateXmlForMultiPartyCallNode(
                            id,
                            ...
                            if (nodeDetails.bHourOption == "anytime") {
                              ...
                              if (nextNodeIfSuccess.length > 0) {
                                ...
                                if (
                                  ...
                                  voice.redirect(
                                    { method: "GET" },
                                    `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${nextNode_for_action_id_data}/${target}`
                                  );
                                  return res.send(voice.toString());
                                }
                              }
                              ...
                              for (
                                ...
                                if (
                                  nodeDetails.bHourDays[i].isWorking === true
                                ) {
                                  const currentTime = new Date();
                                  const currentDay = this.getNameOfDay(
                                    currentTime.getDay()
                                  ...
                                  if (
                                    ...
                                    if (moment(now).isBetween(start, end)) {
                                      ...
                                      if (nextNodeIfSuccess.length > 0) {
                                        ...
                                        if (
                                          ...
                                          if (
                                            ...
                                            console.log(
                                              "data_required_to_filter_conference_details 3290 : ",
                                              data_required_to_filter_conference_details
                                            );
                                            const conference =
                                              new ConferenceModel(
                                        ...
                                        if (
                                          ...
                                          console.log(
                                            "nextNodeIfSuccess[0].data.message : ",
                                            nextNodeIfSuccess[0]
                                          );
                                          ...
                                          const customerVariablesDetails: any =
                                            await IvrStudiosRealTime.findOne({
                                              ParentCallSid: ParentCallSid,
                                            });
                                          let dataBaseVariables = {};
                                          if (customerVariablesDetails) {
                                            // console.log("customerVariablesDetails : ",customerVariablesDetails.variables)
                                            const tempObj =
                                              customerVariablesDetails.variables.map(
                                                (variable: any) => {
                                                  const stringObj = `{"${variable.key}":"${variable.value}"}`;
                                                  return JSON.parse(stringObj);
                                                }
                                          ...
                                          const customer: any =
                                            await IvrStudiosStatusModel.find(
                                              myQuery
                                            );
                                          // console.log("customer : ",customer)
                                          const time =
                                            customer[0].subscribeDate;
                                          const call_back_with_digits =
                                            customer.filter((customer: any) => {
                                              if (
                                                customer.source ===
                                                "gather_response_body_ivr_flow"
                                              ) {
                                          ...
                                          if (customer[0].From) {
                                            if (
                                              customer[0].From.includes("sip:")
                                            ) {
                                              let removed_ip =
                                                customer[0].From.split("@")[0];
                                              removed_sip =
                                                removed_ip.split(":")[1];
                                            } else {
                                              removed_sip = customer[0].From;
                                              removed_sip = removed_sip.replace(
                                                /^0+/,
                                                ""
                                              );
                                              customer_number_only_10_deigits =
                                      ...
                                      console.log(
                                        "if bussiness hour off nextNodeIfSuccess 4602: ",
                                        nextNodeIfSuccess
                                      );
                                      ...
                                      if (
                                        ...
                                        if (nextNodeIfSuccess.length > 1) {
                                          if (
                                            nextNodeIfSuccess[1].type ===
                                            "MessageNode"
                                          ) {
                                            console.log(
                                              "nextNodeIfSuccess[1].data.message : ",
                                              nextNodeIfSuccess[1]
                                            );
                                            let targetNode =
                                              nextNodeIfSuccess[1];
                                            ...
                                            if (customer[0].From) {
                                              if (
                                                customer[0].From.includes(
                                                  "sip:"
                                                )
                                              ) {
                                                let removed_ip =
                                                  customer[0].From.split(
                                                    "@"
                                                  )[0];
                                                removed_sip =
                                                  removed_ip.split(":")[1];
                                        ...
                                        let nextNode_for_action_id_data =
                                          "hangup";
                                        if (nextNode_for_action_id.length > 0) {
                                          nextNode_for_action_id_data =
                                            nextNode_for_action_id[0].target;
                                      ...
                                      if (
                                        ...
                                        if (filterTargetNode.length > 1) {
                                          console.log(
                                            "targetNodeMoreThatnOne : ",
                                            filterTargetNode.length,
                                            filterTargetNode
                                          );
                                          for (
                                            let i = 0;
                                            i < filterTargetNode.length;
                                            i++
                                          ) {
          ...
          if (detailsOfTargetNode.type === "SpeakNode") {
            ...
            voice.redirect(
              { method: "GET" },
              `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${detailsOfTargetNode.target}/${detailsOfTargetNode.id}`
            );
            console.log("xml is : ", voice.toString());
            return res.send(voice.toString());
          }
          ...
          if (detailsOfTargetNode.type === "FunctionRequestNode") {
            const targetNodeForRedirect: any = allNodes.filter((node: any) => {
              if (node.source == detailsOfTargetNode.id) {
                return node.target;
              }
            });
            // console.log("targetNodeForRedirect : ",targetNodeForRedirect)
            const targetNodeOnlyWithSourceAndTarget: any =
              targetNodeForRedirect.map((node: any) => {
                return {
            ...
            console.log(
              "targetNodeOnlyWithSourceAndTarget : ",
              targetNodeOnlyWithSourceAndTarget
            );
            voice.play(
              {
            ...
            const connectorsDetailsInString = encodeURIComponent(
              JSON.stringify(targetNodeOnlyWithSourceAndTarget)
            );
            console.log(
              "connectorsDetailsInString : ",
              connectorsDetailsInString
            );
            voice.redirect(
              { method: "GET" },
              `${conf.BaseUrl}/api/ivrstudios/convert/${id}/checkValue|${connectorsDetailsInString}|${req.body.ParentCallSid}/${detailsOfTargetNode.id}`
            );
            ...
            this.runApiRequestInBackground(
              detailsOfTargetNode,
              parentCallSid,
              req.body.From
            );
          ...
          if (detailsOfTargetNode.type === "MessageNode") {
            ...
            if (filterTargetNode.length === 1) {
              const dataFromVibconnect: any = req.body ? req.body : req.query;
              const xml = await generateXmlForMessage(
                detailsOfTargetNode.data,
                dataFromVibconnect
              );
            ...
            if (filterTargetNode.length > 1) {
              ...
              for (let i = 0; i < filterTargetNode.length; i++) {
                const detailsOfTargetNode = data.input?.find(
                  (node: any) => node.id === filterTargetNode[i].target
                );
        ...
        if (targetNode.type === "SpeakNode") {
          ...
          voice.redirect(
            { method: "GET" },
            `${conf.BaseUrl}/api/ivrstudios/convert/${id}/${targetNode.target}/${targetNode.id}`
          );
          return res.send(voice.toString());
        }
        ...
        if (targetNode.type !== "ivrNode") {
          ...
          const nextNode = allNodes.find(
            (node: any) => node.source === targetNode.target
          );

          if (nextNode.type === "FunctionRequestNode") {
            voice.play(
              { loop: nextNode.data.functionAudioLoop, digits: "wwww3" },
              nextNode.data.functionAudio
            );
            let customer;
            if (req.query.From) {
              customer = req.query.From;
        ...
        if (targetNode.type === "BusinessHourNode") {
          ...
          if (filteredNextNodeData.length > 0) {
            if (nodeDetails.bHourOption == "anytime") {
              ...
              const nextNodeConnectedToBussinessHourNodeOn = connectors.filter(
                (item: any) => {
                  if (
                    item.source === bussinessHourNodeId &&
                    item.sourceHandle == "businesshourOn"
              ...
              const nextNodeIfSuccess = data.input?.filter((item: any) => {
                if (
                  item.id === nextNodeConnectedToBussinessHourNodeOn[0].target
                ) {
                  return item;
                }
              });
              ...
              if (nextNodeIfSuccess.length > 0) {
                ...
                if (nextNodeIfSuccess[0].type === "PlayAudioNode") {
                  ...
                  // console.log("nextNode_for_action : ",nextNode_for_action_id_data)
                  voice.play(
                    { loop: nextNodeIfSuccess[0].data.loop, digits: "wwww3" },
                    nextNodeIfSuccess[0].data.audioUrl
                  );
                  voice.pause({
                    length: nextNodeIfSuccess[0].data.playAudioPause,
                  ...
                  if (
                    ...
                    if (moment(now).isBetween(start, end)) {
                      ...
                      if (nextNodeIfSuccess.length > 0) {
                        ...
                        if (
                          ...
                          if (
                            ...
                            let dataFromVibconnect: any = req.body
                              ? req.body
                              : req.query;
                            const xml =
                              await this.handleMultiPartyCallDistributionOfTypeRoundRobin(
                          ...
                          if (
                            targetNode.data.mpcCallDistribustion === "Parallel"
                          ) {
                            ...
                            if (
                              nextNodeIfSuccess[0].data.mpcCallUsing === "User"
                            ) {
                              console.log(
                                "this is not number this is user typ calling system"
                              );
                            ...
                            if (
                              nextNodeIfSuccess[0].data.callType === "default"
                            ) {
                              From = "+912235315936";
                            }
                            console.log(
                      ...
                      if (nextNodeIfSuccess[0].type === "MultiPartyCallNode") {
                        ...
                        if (
                          targetNode.data.mpcCallDistribustion === "RoundRobin"
                        ) {
                          console.log("write the logic of round robin here ");
                          let dataFromVibconnect: any =
                            JSON.stringify(req.body) !== "{}"
                        ...
                        if (
                          ...
                          if (
                            nextNodeIfSuccess[0].data.mpcCallUsing === "Number"
                          ) {
                            correctNumberArray =
                              nextNodeIfSuccess[0].data.mpcCallUsingNumbers;
                          }
