...
import {addDays, today  ,firstDayOfMonth,lastDayOfMonth,firstDayOfLastMonth } from "../helper/dateHelper";
import {firstMondayOfWeek} from "../helper";
import { getDetails } from '../services/IvrFlowModel';
...
export default class DashboardController extends Controller{
    ...
    public async dashBoardCallStats (req : IRequest , res: IResponse) : Promise<any>{
        ...
        const timeSource : any = req.query.timeSource
        let startDate 
        let endDate 
    ...
    public async hourlyCallStats(req :IRequest , res: IResponse) : Promise<any>{
         
        const AccountSid : any = req.JWTUser?.authId
        ...
        const flowData = await getDetails({AccountSid : AccountSid , fields: ["createdAt" ,  "CallStatus"] ,createdAt : { "$gte" :  new Date(startDate) , "$lte" : new Date(endDate) }})
        ...
        for(let i = 0 ; i < data.length ; i++){
         //console.log("date : ", new Date(data[i].createdAt) , typeof new Date(data[i].createdAt))
            ...
            if(timeZone === 'IST'){
                ...
                const istHours = moment(date).add(5, "hours").add(30, "minutes").hours()
